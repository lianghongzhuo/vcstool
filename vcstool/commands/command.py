...
def add_common_arguments(
    parser, skip_hide_empty=False, skip_nested=False, path_nargs="*", path_help=None
):
    ...
    if not skip_hide_empty:
        group.add_argument(
            ...
            action="store_true",
            default=False,
            help="Hide repositories with empty output",
        )
    if not skip_nested:
        group.add_argument(
            "-n",
...
def simple_main(parser, command_class, args=None):
    ...
    new_jobs = []
    for job in jobs:
        if "_deps" in job["client"].path:
            print("ignore build deps:", job["client"].path)
            continue
        new_jobs.append(job)
    results = execute_jobs(
        new_jobs, show_progress=True, number_of_workers=args.workers, debug_jobs=args.debug
    )
    new_result = []
